...
<!-- Also __note__ that in the masterclass we used to represent features (genes) in columns and examples/data (cells) in rows. The common practice for biological data is to transpose the matrix. -->
S
```{r}
...
# Cell QC
Seurat allows us to easily explore cell QC metrics and `filter` cells based on user defined criteria. 
...
```{r}
pbmc@assays$RNA@data[50:60, 1:5]
```
...
```{r}
FeaturePlot(pbmc, dims = c(1, 2), features = c("nFeature_RNA"))
```
